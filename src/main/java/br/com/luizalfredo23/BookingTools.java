...
public class BookingTools {
	...
	public String getBookingDetails(long bookingId) {
		return bookingService.getBookingDetails(bookingId).map(Booking::toString)
	...
	public String cancelBooking(long bookingId) {
		return bookingService.cancelBooking(bookingId).map(b -> "Reserva " + b.id() + " cancelada com sucesso.")
	...
	public String listPackagesByCategory(Category category) {
		...
		return "Pacotes encontrados para a categoria '" + category + "': "
				+ packages.stream().map(Booking::destination).toList().toString();
	}
